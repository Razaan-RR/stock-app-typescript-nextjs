...
export const searchStocks = cache(
  async (query?: string): Promise<StockWithWatchlistStatus[]> => {
    try {
      ...
      return results
        .map((r) => {
          const rr = r as FinnhubSearchResultWithExchange
          const upper = (rr.symbol || '').toUpperCase()
          const exchange = rr.displaySymbol || rr.__exchange || 'US'
          return {
            symbol: upper,
            name: rr.description || upper,
            exchange,
            type: rr.type || 'Stock',
            isInWatchlist: false,
          }
